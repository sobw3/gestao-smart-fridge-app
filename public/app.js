document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FUNÇÕES DE CÁLCULO E DADOS (REVISADO) ---
    ...
    const calculatePdvMetrics = (pdvId, startDate = null, endDate = null) => {
        ...
        if (!pdv) return { grossProfit: 0, finalProfit: 0, stockValueCost: 0, stockValueResale: 0, salesCount: 0, ticket: 0, revenue: 0 };
        
        const relevantSales = filterByDate(state.sales, 'date').filter(s => s.pdvId === pdvId);
        
        // Lucro Bruto = Valor total vendido - Custo dos produtos vendidos
        ...
        const grossProfit = totalSoldValue - costOfGoods;

        // Faturamento (para metas) = Apenas vendas que entram no caixa (PIX)
        const revenue = relevantSales
            .filter(s => s.paymentMethod === 'pix' || s.paymentMethod === 'outros')
            .reduce((sum, s) => sum + s.totalPrice, 0);

        // Custos = Soma de custos fixos + custos variáveis (como compras de estoque)
        ...
        // Lucro Líquido/Final = Lucro Bruto - Todos os outros custos
        const finalProfit = grossProfit - (fixedCosts + variableCostsInPeriod);
    ...
    function renderDashboard() {
        const totalMetrics = (state.pdvs || []).reduce((totals, pdv) => {
            ...
            totals.finalProfit += metrics.finalProfit; // Usar o lucro final para o total
            return totals;
        }, { revenue: 0, finalProfit: 0 });
        ...
        const summaryCards = `
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
                ...
                <div class="bg-gray-800 p-5 rounded-xl shadow-lg"><h3 class="text-sm font-medium text-gray-400">Lucro Líquido Total</h3><p class="mt-1 text-3xl font-semibold  ${totalMetrics.finalProfit >= 0 ? 'text-green-500' : 'text-red-500'}">${formatCurrency(totalMetrics.finalProfit)}</p></div>
                <div class="bg-gray-800 p-5 rounded-xl shadow-lg"><h3 class="text-sm font-medium text-gray-400">Saldo em Carteira</h3><p class="mt-1 text-3xl font-semibold text-cyan-500">${formatCurrency(state.digitalWallet?.balance || 0)}</p></div>
                <div class="bg-gray-800 p-5 rounded-xl shadow-lg"><h3 class="text-sm font-medium text-gray-400">Crédito a Receber</h3><p class="mt-1 text-3xl font-semibold text-yellow-500">${formatCurrency(state.smartCredit?.receivable || 0)}</p></div>
            </div>`;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (!name || isNaN(investment)) {
                    Swal.fire('Erro', 'Nome e investimento inicial são obrigatórios.', 'error');
                    return;
                }
                const newPdv = { id: generateId(), name, initialInvestment: investment, fixedCosts: [], variableCosts: [], inventory: [] };
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-700/50 p-4 rounded-lg">
                            ...
                            <ul class="mt-2 space-y-2 text-sm">
                                <li class="flex justify-between"><span>Lucro Bruto (Vendas):</span> <span class="font-bold text-green-400">${formatCurrency(metrics.grossProfit)}</span></li>
                                <li class="flex justify-between"><span>Lucro Final (Líquido):</span> <span class="font-bold text-xl ${metrics.finalProfit >= 0 ? 'text-green-500' : 'text-red-500'}">${formatCurrency(metrics.finalProfit)}</span></li>
                ...
                <table class="w-full text-sm text-left text-gray-300">
                    ...
                    <tbody>
                        ${(state.clients || []).map(client => {
                            ...
                            `
                        }).join('') || '<tr><td colspan="5" class="text-center py-8 text-gray-400">Nenhum cliente cadastrado.</td></tr>'}
                    </tbody>
                </table>
            </div>
        `;
    ...
    const showAddClientModal = () => {
        Swal.fire({
            ...
            if (result.isConfirmed && result.value.name) {
                const { name, creditLimit } = result.value;
                if (!state.clients) state.clients = [];
                state.clients.push({ id: generateId(), name, creditLimit, debt: 0 });
                saveDataToServer();
                render();
            }
        });
    ...
    const handleCreditSale = (pdvId, productId, quantity, totalPrice) => {
        if (!state.clients || state.clients.length === 0) {
            Swal.fire('Ação Necessária', 'Cadastre um cliente na página "Clientes" antes de vender no crédito.', 'info');
            return;
        }
        ...
        Swal.fire({
            ...
            html: `<p class="mb-4">Selecione o cliente:</p>
    ...
    const processSale = (pdvId, productId, quantity, paymentMethod, clientId = null) => {
        ...
        const newSale = { id: generateId(), pdvId, productId, quantity, unitPrice: product.resalePrice, totalPrice, costAtTimeOfSale: product.currentCost, date: new Date().toISOString(), paymentMethod, clientId };
            ...
            return `
                <div class="bg-gray-800 p-4 rounded-lg flex justify-between items-center ${isFullyPaid ? 'opacity-50' : ''}">
                    <div>
                        ...
                        <p class="text-sm text-gray-400">${item.pdvId ? getPdv(item.pdvId)?.name : 'Geral'} | ${!isClientDebt ? `Venc.: ${new Date(item.dueDate).toLocaleDateString('pt-BR')}`: 'Dívida em aberto'}</p>
                    </div>
    ...
    const showAddFinanceModal = (type) => {
        ...
        Swal.fire({
            ...
            if(result.isConfirmed && result.value.description && !isNaN(result.value.amount)) {
                ...
                if (!list) list = [];
                list.push({ id: generateId(), ...result.value, paid: false, payments: [] });
                saveDataToServer();
                render();
            }
        });
    ...
    const handleGenerateReport = (e) => {
        ...
        const relevantSales = (state.sales || []).filter(s => {
            ...
            const isInDateRange = saleTime >= startDateTime && saleTime <= endDateTime;
        ...
        const contasPagas = centralTransactions.filter(t => {
            ...
            return t.type === 'payable' && tTime >= startDateTime && tTime <= endDateTime;
        });
        const totalPago = contasPagas.reduce((sum, t) => sum + t.amount, 0);
    
        const contasRecebidas = centralTransactions.filter(t => {
            const tTime = new Date(t.date).getTime();
            return t.type === 'receivable' && tTime >= startDateTime && tTime <= endDateTime;
        });
        ...
        const custoProdutos = relevantSales.reduce((sum, s) => sum + (s.costAtTimeOfSale * s.quantity), 0);
        const lucroBruto = faturamentoBruto - custoProdutos;
        ...
        const depositosCarteira = centralTransactions.filter(t => {
             const tTime = new Date(t.date).getTime();
             return t.type === 'wallet_deposit' && tTime >= startDateTime && tTime <= endDateTime;
        ...
        document.getElementById('report-results').innerHTML = `
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-gray-700 p-5 rounded-lg">
                    <p class="text-sm text-gray-400">Lucro Bruto (Vendas - Custo Produtos)</p>
                    <p class="text-3xl font-bold text-green-400">${formatCurrency(lucroBruto)}</p>
                </div>
                 <div class="bg-gray-700 p-5 rounded-lg">
